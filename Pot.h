#ifndef POT_H
#define POT_H


/* The actual max value from an analog input is 1024, however, if we want to use
 * a different max value, say 127, we put that as the numerator.  This also
 * allows a bit of debounce when reducing the resolution. */
#define CONVERT          0.12305  // conversion factor from 1024 to 126
#define GET_MAX_VALUE(v) (uint8_t)round(v * CONVERT)
#define SET_MAX_VALUE(v) (uint16_t)round(v / CONVERT)
#define WINDOW           10 // leeway to go from set value to pot value


struct Pot {
  uint16_t val, old_val, new_val;
  uint8_t  pin, is_set;

  void setup(uint8_t pin) {
    this->pin = pin;
    val = old_val = analogRead(pin);
    is_set = false;
  }

  void loop() {
    new_val = analogRead(pin);

    if ( is_set ) {
      if ( old_val > new_val + WINDOW || old_val < new_val - WINDOW ) {
        is_set = false;
      }
      //digitalWrite(13, true);
    }
    else {
      val = new_val; 
      //digitalWrite(13, false);
    }

  }

  void set_value(uint8_t x) {
    val    = SET_MAX_VALUE(x);
    is_set = true;
  }

  uint16_t get_value() {
    return GET_MAX_VALUE(val);
  }
};


#undef CONVERT
#undef GET_MAX_VALUE
#undef SET_MAX_VALUE
#undef WINDOW

#endif
